...
function getAvgByGroup(data, groupKey, valKey) {
    const sums = {}, counts = {};
    data.forEach(d => {
        if(!sums[d[groupKey]]) { sums[d[groupKey]] = 0; counts[d[groupKey]] = 0; }
...
function renderRadarProfile(data) {
    ...
    new Chart(document.getElementById('chartRadarProfile'), {
        type: 'radar',
        data: {
            labels: ['Physical Energy', 'Social Battery', 'Sport Activity', 'Screen Time'],
            datasets: [
                {
                    label: 'Early Birds ☀️',
                    data: getStats(birds),
    ...
    new Chart(document.getElementById('chartBubbleComplex'), {
        ...
        options: {
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: (c) => `${c.raw.type} | Soc: ${c.raw.x} | Scrn: ${c.raw.y} | Sleep: ${(c.raw.r/1.5).toFixed(1)}h`
                    }
                }
            },
            scales: {
                x: { title: { display: true, text: 'Social Battery (Introvert -> Extrovert)' }, min: 0, max: 11 },
                y: { title: { display: true, text: 'Screen Time (Hours)' }, min: 0, max: 12 }
            }
        }
    });
}
...
function renderScatterTravelTech(data) {
    const dataset = data.map(d => ({ x: d.COUNTRIES_VISITED, y: d.SCREEN_TIME }));
...
function renderBarSocialSport(data) {
    ...
    const sums = {}, counts = {};
    for(let i=1; i<=10; i++) { sums[i]=0; counts[i]=0; } // init
